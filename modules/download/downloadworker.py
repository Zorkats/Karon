...
class DownloadWorker(QThread):
    ...
    async def process_downloads(self):
        ...
        for index, doi in enumerate(dois):
            ...
            if doi.startswith("10.1007"): # Intentar descargar con API de Springer
                ...
                if api_key:
                    ...
                    if pdf_content:
                        ...
                        with open(download_file_path, 'wb') as f:
                            f.write(pdf_content)
                        self.log_signal.emit(f"Downloaded PDF for {doi} via Springer API")
                        self.progress_signal.emit(int((index + 1) / total_dois * 100))
                        api_used = True
                        continue

            if doi.startswith("10.1145"): # ACM
                self.log_signal.emit(f"Intentando descargar con la API de ACM para {doi}...") 
                api_key = self.config.get("acm_api", None)
                if api_key:
                    pdf_content = download_pdf_via_api(doi, api_key)
                    if pdf_content:
                        download_file_path = os.path.join(self.download_path, f"{doi.replace('/', '_')}.pdf")
                        with open(download_file_path, 'wb') as f:
                            f.write(pdf_content)
                        self.log_signal.emit(f"Downloaded PDF for {doi} via ACM API")
                        self.progress_signal.emit(int((index + 1) / total_dois * 100))
                        api_used = True
                        continue

            if doi.startswith("10.1109"): # IEEE
                ...
                if api_key:
                    ...
                    if pdf_content:
                        ...
                        with open(download_file_path, 'wb') as f:
                            f.write(pdf_content)
                        self.log_signal.emit(f"Downloaded PDF for {doi} via IEEE API")
                        self.progress_signal.emit(int((index + 1) / total_dois * 100))
                        api_used = True
                        continue
